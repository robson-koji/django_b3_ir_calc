...
class Stock:
    ...
    def __init__(self, ticker, start, end=datetime.datetime.now()):
        ...
        df = pd.DataFrame(list(Cotacoes.objects.filter(stock=ticker).order_by('-id')[:2500].values()))
        ...
        # stockData = sr[-14:]
        stockData = sr
        stockData.rename(columns={'close':'Close', 'high':'High', 'low':'Low', 'open':'Open', 'volume':'Volume'}, inplace=True)
        stockData = stockData.apply(pd.to_numeric, downcast='float')
